...
def save_analysis_result(original_input, confidence, label, extracted_text=None, data_type="AI Analysis", ai_score=None, expert_score=None, title="N/A", link="N/A", subject="General"):
    try:
        ...
        # 2. Prevent exact global duplicates for same type
        search_text = clean_input.lower()
        if history and len(history) > 0:
            # Check if this exact input has been analyzed before
            existing_inputs = [h.get("original_input", "").strip().lower() for h in history]
            if search_text in existing_inputs:
                print(f"[*] SKIPPING: Global duplicate for {data_type} detected. Already in history.")
                return True # Return true because it's technically already saved
...
def deep_fact_check():
    ...
    try:
        ...
        for claim in claims_list:
            label, conf, explanation, evidence_sources = verify_claim(claim)
            results.append({
                "claim": claim,
                "label": label,
                "confidence_score": conf,
                "explanation": explanation,
                "evidence_summaries": evidence_sources[:3] # Returning Top 3 reliable URLs
            })
                    ...
                    for item in history:
                        try:
                            ...
                            if item_date >= start_of_week:
                                day_idx = item_date.weekday() # 0=Mon, 6=Sun
